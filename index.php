...
if (isset($_POST['type'])) {
    $_SESSION['selected_transaction_type'] = $_POST['type'];
}
...
<?php
// Prepare $days and $amounts as in your Chart.js code
...
window.onload = function() {
    var chart = new CanvasJS.Chart("chartContainer", {
        ...
        data: [{
            type: "line",
            color: "#007bff", // Blue line
            lineThickness: 3,
            markerType: "circle",
            markerColor: "#007bff", // Blue points
            markerSize: 8,
    ...
    <div class="card-body">
      <form method="post" class="row g-3 align-items-end" id="reminderForm">
        ...
        <div class="col-md-5" id="toDateDiv" style="display:none;">
          ...
          <input type="date" name="loanToDate" id="loanToDate" class="form-control"value="<?= htmlspecialchars($selected_date); ?>">
        </div>
      ...
      <ul class="mb-0">
        <?php foreach ($important_reminders as $rem): ?>
          <li>
            ...
            <?php if($rem['date'] != $rem['to_date']){ ?>
            <span class="badge bg-warning text-white"><?= htmlspecialchars($rem['date']) ?> to <?= htmlspecialchars($rem['to_date']) ?></span>
            <?php } ?>
            <form method="post" class="d-inline">
